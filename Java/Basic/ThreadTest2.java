...
class SnailThread extends Thread
{
	...
	public void run() {//스레드가 할 일
		for(int i=0;i<5;i++) {
			...
			/*try {
			Thread.sleep(1000);//1/1000초 단위 ==> Block상태가 된다(sleep(), I/o작업시)
			//1초간 잠을 잔다
			}catch(InterruptedException e) {//<= 스레드에 interrupt()메서드가 호출될때 예외가 발생함
				e.printStackTrace();
				break;
			}*/
...
public class ThreadTest2 {
	public static void main(String[] args) {
		...
		try {
			tr2.join();
			//tr2스레드가 작업을 다 마칠때까지 tr2.join()호출한 스레드(여기서는 main스레드)가
			//block상태가 된다
